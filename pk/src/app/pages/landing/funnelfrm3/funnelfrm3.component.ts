...
@Component({
  selector: "app-funnelfrm3",
  templateUrl: "./funnelfrm3.component.html",
  styleUrls: ["./funnelfrm3.component.css"]
})
export class Funnelfrm3Component implements OnInit {
  index: number = 1;
  loading = false;
  promoForm: FormGroup;
  erromsg: any;
  ...
  ngOnInit() {
    ...
    this.promoForm = this.fb.group({
      promo_badge_id: ["", Validators.required],
    ...
    if (this.logeduser) {
      console.log(this.logedemail + "-emial");
    }
  }
  ...
  nextStep(index) {
    ...
    if (this.index == 1 && this.f["promo_badge_id"].valid) {
      this.index = this.index + 1;
    } else if (this.index == 2 && this.f["promo_objective"].valid) {
      this.index = this.index + 1;
    } else if (this.index == 3 && this.f["promo_type"].valid) {
      this.index = this.index + 1;
    } else if (
      this.index == 4 &&
      (this.f["title"].valid &&
