...
export class SubcateComponent implements OnInit {
  ...
  locat_address() {
    this.http.get(this.locip).subscribe((res: any) => {
      ...
      this.http
        .get(
          ...
          res.ip +
          "?access_key=" +
          environment.location_apikey
        )
        .subscribe((res1: any) => {
          this.addfetched = "t";
          this.form_getaddress = res1.region_code + ", " + res1.region_name;
          var locs = res1.region_code + ", " + res1.region_name;
    ...
    if (frm_phone == true && frm_msg == true) {
      ...
      if (this.logeduser) {
        ...
        sessionStorage.setItem(
          "re_cate",
          "listing-category/" + this.parentSlug + "/" + this.cateSlug
        );
  ...
  getListing(slug) {
    ...
    this.http
      ...
      .subscribe((res: any) => {
        ...
        this.subcate(res.category.parent);
        this.parentdet = res.parent;
        this.listingRegion = res.regions;
        this.listSubcate = res.categories;

        this.cateForm.patchValue({
          buss_cate: this.cateName.parent,
          cateid: this.cateName.id,
          buss_scate: this.cateName.id
        });

        this.bannerthumb = res.parent.thumbnail;
      });
  }
  ...
  onSearch() {
    ...
    /** var elmnt = document.getElementById("searchresult_li");
  ...
  twitterfeeds() {
    const tid = environment.twitterfeed + this.parentSlug;
